...
class ElastiPymemcache(PyMemcacheCache):
    ...
    def _validate_endpoint(self) -> str:
        if not self._servers or len(self._servers) != 1:
            raise InvalidCacheBackendError(
                "ElastiCache requires exactly one Configuration Endpoint (host:port)."
            )

        endpoint = self._servers[0]
        if not isinstance(
            endpoint, str
        ) or not _AWS_CONFIGURATION_ENDPOINT_PATTERN.fullmatch(endpoint):
            raise InvalidCacheBackendError(
                f"Invalid Configuration Endpoint '{endpoint}'. Expected 'host:port'."
            )
        return endpoint
